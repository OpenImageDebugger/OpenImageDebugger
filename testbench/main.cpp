...
public:
  ...
  template <typename T> void create(int rows_, int cols_, int channels_) {
    cols = cols_;
    rows = rows_;
...
template <typename T> void fillBuffer(int W, int H, int C, Mat &matrix) {
  ...
  for (int n = 0; n < nN; ++n) {
    ...
    for (int y = 0; y < H; ++y) {
      for (int x = 0; x < W; ++x) {
        float r =
            sin(cos(ni) * 10 + x / static_cast<float>(W) * M_PI) * 127 + 128;
        float g =
            sin(cos(ni) * 20 + y / static_cast<float>(H) * M_PI) * 127 + 128;
        float b =
            cos(cos(ni) * 30 + x / static_cast<float>(W) * M_PI) * 127 + 128;
        r = x;
        g = y;
...
template <typename T> void ones(int W, int H, int C, Mat &matrix) {
  assert(W > 0 && H > 0 && C > 0);
  matrix.create<T>(H, W, C);
...
template <typename T> void computeSumTable(int W, int H, int C, Mat &matrix) {
  assert(W > 0 && H > 0 && C > 0);
  Mat output;
  ...
  auto kernelH = [&W, &H, &C](Mat::Iterator<T> &in, Mat::Iterator<T> &out,
                              int iteration) {
  ...
  auto kernelV = [&W, &H, &C](Mat::Iterator<T> &in, Mat::Iterator<T> &out,
                              int iteration) {
    int step = 1 << iteration;
  ...
  auto callKernels = [&W, &H, &in, &out, &kernelH, &kernelV,
                      &isOutputSwapped](bool isHorizontal) {
    int numIterations;
    if (isHorizontal) {
...
void doSimpleCalculation(int W, int H, int C, float offset, Mat &matrix) {
  ...
  for (int y = 0; y < H; ++y) {
    for (int x = 0; x < W; ++x) {
      for (int c = 0; c < C; ++c) {
        out(y, x, c) =
            (sin(x / 64.0f + offset + y / (float)H * offset) * 0.5f + 0.5f) *
            255;
      }
    }
  }
...
void changeStackOnPurpose() { cout << "Changed stack position" << endl; }
