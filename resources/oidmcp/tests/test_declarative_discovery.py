...
def test_document_recursion_error_warns_and_skips_file(tmp_path, caplog,
    ...
    def raise_recursion_error(file_object):
        raise RecursionError('maximum recursion depth exceeded')

    monkeypatch.setattr(declarative.json, 'load', raise_recursion_error)
    assert declarative._load_types_file(str(types_file)) == []
    assert any(record.levelno == logging.WARNING
               for record in caplog.records)
...
def test_entry_recursion_error_skips_entry_rest_of_file_loads(tmp_path,
    ...
    def validate_entry_or_raise(entry):
        if entry.get('name') == 'Pathological':
            raise RecursionError('maximum recursion depth exceeded')
        return original_validate_entry(entry)

    monkeypatch.setattr(declarative, '_validate_entry',
                        validate_entry_or_raise)
    pathological = dict(VALID_DOC['types'][0], name='Pathological',
                        match='^Pathological$')
    document = {'version': 1,
                'types': [pathological, VALID_DOC['types'][0]]}
    types_file = tmp_path / 'types.json'
    write_doc(types_file, document)
    inspectors = declarative._load_types_file(str(types_file))
    assert [inspector.name for inspector in inspectors] == ['MyImage']
    assert any('Pathological' in record.message
               for record in caplog.records
               if record.levelno == logging.WARNING)
...
def test_type_bridge_get_inspectors_lists_registered(monkeypatch, tmp_path):
    monkeypatch.delenv('OID_TYPES_PATH', raising=False)
    monkeypatch.chdir(tmp_path)          # no workspace .oid/types.json
    from oidscripts import typebridge

    inspectors = typebridge.TypeBridge().get_inspectors()

    assert isinstance(inspectors, list)
    names = [getattr(insp, 'name', type(insp).__name__) for insp in inspectors]
    assert 'cv::Mat' in names            # a JSON built-in is registered


def test_conformance_compare_labels_each_inspector(capsys):
    from oidscripts import conformance
    from oidscripts.oidtypes import declarative

    json_inspector = declarative.load_builtin_inspectors()[0]   # cv::Mat

    class FakePy:
        def is_symbol_observable(self, symbol_obj, symbol_name):
            return True

        def get_buffer_metadata(self, obj_name, picked_obj, bridge):
            raise RuntimeError('unsupported here')

    class FakeTypeBridge:
        def get_inspectors(self):
            return [json_inspector, FakePy()]
    ...
    conformance.register(FakeBridge(), FakeTypeBridge())
    conformance.compare('img')

    out = capsys.readouterr().out
    assert 'JSON:cv::Mat' in out          # DeclarativeInspector → JSON label
    assert 'python:FakePy' in out         # everything else → python label


def test_conformance_compare_reports_evaluation_failure(capsys):
    from oidscripts import conformance
    from oidscripts.oidtypes import declarative
    ...
    class FakeTypeBridge:
        def get_inspectors(self):
            return [json_inspector]

    class FailingBridge:
        def evaluate_expression(self, expression):
            raise RuntimeError('no stopped frame')

    conformance.register(FailingBridge(), FakeTypeBridge())
    conformance.compare('img')

    out = capsys.readouterr().out
    # A lookup failure is reported once and returns, never raised, and no
    # inspector line is printed because no object was picked.
    assert "ERROR evaluating 'img': no stopped frame" in out
    assert 'JSON:cv::Mat' not in out
