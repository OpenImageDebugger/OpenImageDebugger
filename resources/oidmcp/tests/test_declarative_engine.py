...
def test_if_condition_non_boolean_result_raises_entry_error():
    ...
    with pytest.raises(declarative.EntryEvaluationError) as excinfo:
        declarative._resolve_node(resolution, node, declarative._leaf_int)
    assert excinfo.value.entry_name == 'TestEntry'
    assert excinfo.value.field == 'channels'
...
def test_validate_rejects_min_wrapper_pointer_candidate():
    ...
    entry = dict(FLOOR_ENTRY,
                 pointer={'first_valid': [{'expr': '{sym}.data', 'min': 1},
                                          '{sym}.data']})
    errors = declarative._validate_entry(entry)
    assert any('pointer' in error and 'min' in error for error in errors)
...
def test_resolve_pixel_layout_if_condition_non_boolean_raises_entry_error():
    # Same _to_bool-escape hazard as _resolve_node's 'if' handling, but on
    # the pixel_layout walk's own condition evaluation.
    bridge = RecordingBridge({'(img).flag': 'sideways'})
    resolution = make_resolution(bridge, field='pixel_layout')
    node = {'if': '{sym}.flag', 'then': 'bgra', 'else': 'rgba'}
    with pytest.raises(declarative.EntryEvaluationError) as excinfo:
        declarative._resolve_pixel_layout(resolution, node)
    assert excinfo.value.entry_name == 'TestEntry'
    assert excinfo.value.field == 'pixel_layout'
...
def test_first_valid_min_runs_leaf_and_falls_through_on_invalid_dtype():
    ...
    resolution = make_resolution(bridge, field='dtype')
    candidates = [{'expr': '{sym}.bogus_code', 'min': 1}, '{sym}.depth']
    assert declarative._resolve_first_valid(
        resolution, candidates,
        declarative._leaf_dtype) == symbols.OID_TYPES_UINT8
    assert bridge.requests == ['(img).bogus_code', '(img).depth']
...
def test_first_valid_min_runs_leaf_and_rejects_negative_with_negative_min():
    ...
    assert declarative._resolve_first_valid(
        resolution, candidates, declarative._leaf_int) == 6
    assert bridge.requests == ['(img).signed_dim', '(img).rows']


def test_first_valid_min_returns_leaf_result_for_valid_value():
    # Happy path: a {expr, min} value that clears `min` and passes the leaf is
    # returned via the leaf (which validates and normalizes it), not bypassed.
    bridge = RecordingBridge({'(img).code': symbols.OID_TYPES_FLOAT32})
    resolution = make_resolution(bridge, field='dtype')
    candidates = [{'expr': '{sym}.code', 'min': 0}]
    assert declarative._resolve_first_valid(
        resolution, candidates,
        declarative._leaf_dtype) == symbols.OID_TYPES_FLOAT32
