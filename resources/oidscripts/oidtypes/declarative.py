...
def _condition_bool(resolution, expr):
    """
    Evaluate an 'if' condition and interpret the result as a boolean,
    wrapping a _to_bool failure in EntryEvaluationError -- the same
    pattern _leaf_bool uses for boolean field leaves -- so a condition
    result _to_bool cannot interpret carries entry/field context instead
    of escaping as a bare ValueError.
    """
    value = resolution.evaluate(expr)
    try:
...
def _resolve_node(resolution, node, leaf):
    ...
    if isinstance(node, dict):
        ...
        if 'if' in node:
            condition = _condition_bool(resolution, node['if'])
    ...
    for entry in entries:
        ...
        try:
            ...
            log.warning(f'Skipping types entry {name!r} from {path}: '
                        'value node is too deeply nested')
            continue
        if problems:
            log.warning(f'Skipping types entry {name!r} from {path}: '
                        + '; '.join(problems))
